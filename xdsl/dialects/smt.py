...
from xdsl.ir import Attribute, ParametrizedAttribute, SSAValue, TypeAttribute
...
class ConstantBoolOp(IRDLOperation):
    ...
    def __init__(self, value: bool):
        ...
        super().__init__(properties={"value": value_attr})
...
def parse_same_operand_type_variadic_to_bool_op(
    parser: Parser,
...
def print_same_operand_type_variadic_to_bool_op(
...
class DistinctOp(IRDLOperation):
    ...
    def parse(cls, parser: Parser) -> DistinctOp:
        operands, attr_dict = parse_same_operand_type_variadic_to_bool_op(parser)
        op = DistinctOp(operands=operands)
        op.attributes = attr_dict
        return op

    def print(self, printer: Printer):
        print_same_operand_type_variadic_to_bool_op(
